...
import { REPORT_TYPES } from "./constantsRecordManagementStatistics";
...
const RecordManagementStatistics = (props) => {
  ...
  const handleExport = useCallback(async (exportType, filtersToExport) => {
    try {
      setLoading(true);
      const fileName = selectedOption?.label || "Báo cáo thống kê hồ sơ";
  ...
  if (open) {
    return (
      <Swipper
        open={open}
        onClose={onClose}
        title={title || "Báo cáo thống kê hồ sơ lưu trữ"}
        type="view"
      >
        {content}
      </Swipper>
...
export default RecordManagementStatistics;
