...
useEffect(() => {
  if (!open || !initialFileId) return;

  const fetchData = async () => {
    try {
      const res = await api.get(`${API_GET_DATA_FILE}/${initialFileId}`);
      const data = res.data;
      handleSelectFolder(data);
    } catch (err) {
      toast("Không load được dữ liệu hồ sơ", "error");
    }
  };

  fetchData();
}, [open, initialFileId, handleSelectFolder]);
...
//   const handleOpenFolderDialog = () => setOpenFolderDialog(true);
  ...
  //     handleSelectFolder(initialFileId);
  //   }
  // }, [open, initialFileId, handleSelectFolder]);
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                ...
                {/* Search button could be added here if needed for Profile management */}
              </div>
              <StyledDivider />
            </Grid>
            ...
            {/* {!hiddenSearchCategory && (
              <StyledSearchButtonWrapper item xs={12} md={4}>
...
OpenProfileManagement.propTypes = {
...
export default withSharedComponents(OpenProfileManagement);
