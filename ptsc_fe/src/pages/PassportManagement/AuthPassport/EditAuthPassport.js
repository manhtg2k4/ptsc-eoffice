...
const EditAuthPassport = (props) => {
	...
	const initialByPermissionMemberListRef = useRef([]);
	const previousScopeRef = useRef("");
	const isScopeSoftClearedRef = useRef(false);
	...
	useEffect(() => {
		...
		const dataDetail = async () => {
			try {
				...
				draftPermissionIdRef.current = res?.id || res?._id || props?.id || null;

				const officers = Array.isArray(res?.officerList)
					? res.officerList
					: [];
				const fetchedScopeValue = typeof res?.passportBorrowScope === "object"
					? res?.passportBorrowScope?.value
					: res?.passportBorrowScope || "";

				const mappedMembers = officers.length > 0
				...
				if (fetchedScopeValue === "byPermissionList") {
					initialByPermissionMemberListRef.current = mappedMembers;
				} else {
					initialByPermissionMemberListRef.current = [];
				}
				previousScopeRef.current = fetchedScopeValue || "";
				isScopeSoftClearedRef.current = false;
				reset({ ...res });
			} catch (error) {
				const messageError =
		...
		dataDetail();
	}, [createEmptyMember, dispatch, open, reset, onClose, toast, props?.id]);
	...
	const handleSave = useCallback(async (data) => {
		try {
			...
			const officerListPayload = scopeValue === "byPermissionList" ? validOfficerList : [];
			const body = {
				...data,
				...
				passportBorrowScope: typeof data?.passportBorrowScope === "object"
					? data?.passportBorrowScope?.value
					: data?.passportBorrowScope,
				officerList: officerListPayload,
			}
			...
			toast("Cập nhật quyền mượn hộ chiếu thành công!", "success");
			reset(defaultValueAuthPassport);
			onSuccess?.();
			onClose();
		} catch (error) {
	...
	useEffect(() => {
		if (!open) return;
		if (!passportBorrowScopeValue) return;

		const previousScope = previousScopeRef.current;
		if (!previousScope) {
			previousScopeRef.current = passportBorrowScopeValue;
			return;
		}

		if (previousScope === passportBorrowScopeValue) return;

		if (previousScope === "byPermissionList" && passportBorrowScopeValue !== "byPermissionList") {
			setMemberList([createEmptyMember()]);
			setValue("officerList", [], {
				shouldDirty: true,
				shouldTouch: true,
			});
			clearErrors("officerList");
			isScopeSoftClearedRef.current = true;
		}

		if (
			previousScope !== "byPermissionList" &&
			passportBorrowScopeValue === "byPermissionList" &&
			isScopeSoftClearedRef.current
		) {
			const restoredMembers =
				initialByPermissionMemberListRef.current.length > 0
					? initialByPermissionMemberListRef.current
					: [createEmptyMember()];
			setMemberList(restoredMembers);
			isScopeSoftClearedRef.current = false;
		}
	...
	}, [memberList, setValue, getEntityId, clearErrors, isByPermissionList]);
	...
	return (
		<BaseSwipper
			title={title || "Cập nhật phân quyền hộ chiếu"}
			open={open}
			onClose={handleClose}
...
export default withSharedComponents(EditAuthPassport);
