...
function ViewNewsCancelled({ open, onClose, sharedComponents, newsId }) {
  const {
  ...
  const renderCancelReasonField = useCallback(
    ({ field }) => (
      <InputComponents
        label="LÝ DO HỦY TIN"
    ...
    [errors?.cancelReason, isEditMode]
  );

  const renderCancellerNameField = useCallback(
    ({ field }) => (
      <InputComponents
        label="NGƯỜI HỦY TIN"
        placeholder="Người thực hiện hủy tin"
        disabled
        error={!!errors?.cancellerName}
        helperText={errors?.cancellerName?.message}
        {...field}
      />
    ),
    [errors?.cancellerName]
  );
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} sm={6}>
                <Controller
                  name="cancellerName"
                  control={control}
                  render={renderCancellerNameField}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <Controller
                  name="cancelReason"
                  control={control}
                  render={renderCancelReasonField}
                />
              </Grid>
            </Grid>
...
export default withSharedComponents(ViewNewsCancelled);