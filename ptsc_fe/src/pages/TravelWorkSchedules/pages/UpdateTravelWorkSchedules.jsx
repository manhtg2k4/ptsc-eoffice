...
// --- MAIN COMPONENT ---
const UpdateTravelWorkSchedules = ({
	sharedComponents,
	onClose,
	setReloadData,
	data,
	...
	logger.log("optionScheduleType", optionScheduleType);
	...
	const scheduleTypeRaw = watch("scheduleType");
	const scheduleType = getOptionValue(scheduleTypeRaw);
	...
	//     console.log("TravelWork Update Errors:", errors);
	//   }
	// }, [errors]);
	...
	// Auto-set calendarFormat and handle multi-day defaults (Synced with Create behavior)
	useEffect(() => {
		...
		if (isSingleDay) {
			// Only set default if value is missing or currently invalid according to CRM sources
			if (!calendarFormat || !currentIsValid) {
				if (desiredExists) {
					setValue("calendarFormat", desired, { shouldDirty: false });
		...
		if (isMultiDay) {
			if (calendarFormat !== "fullDay") {
				setValue("calendarFormat", "fullDay", { shouldDirty: false });
			}

			// Auto-set travelSchedule to first option if not set
			if (!travelSchedule && optionTravelSchedule.length > 0) {
				setValue("travelSchedule", getOptionValue(optionTravelSchedule[0]), { shouldDirty: false });
			}

			// Auto-append first multi-day schedule item if array is empty
			if (fields.length === 0) {
				append({
					numDays: getOptionValue(optionNumDays[0]) || "motngay",
					format: "session",
		...
		isMultiDay,
		travelSchedule,
		optionTravelSchedule,
		optionNumDays,
		calendarFormat,
		optionCalendarFormat,
		fromDate,
		toDate,
		setValue,
		fields.length,
	...
	const onSubmit = (formData) => {
		if (
			formData.scheduleType === "singleDay" &&
			getOptionValue(formData.calendarFormat) === "session"
		) {
			const isMorningEmpty =
				!formData.morningLocation?.trim() && !formData.morningContent?.trim();
			const isAfternoonEmpty =
				!formData.afternoonLocation?.trim() &&
				!formData.afternoonContent?.trim();
		...
		if (isMultiDay && checkDateOverlaps(formData.schedules)) {
			toast(
	...
	const handleConfirmSave = async () => {
		...
		try {
			...
			await dispatch(
				updateDataTravelWork({ data: payload, id: data.id })
			).unwrap();
			toast("Cập nhật lịch công tác thành công!", "success");
			...
			toast(
				...
				"Có lỗi xảy ra khi cập nhật lịch công tác!",
				"error"
			);
		} finally {
	...
	return (
		...
		>
            ...
            <SectionHeader>
              <SkyTypography color="error" variant="body2" sx={{ mb: 2 }}>
                {errors.schedules?.message || errors.schedules?.root?.message}
              </SkyTypography>
            </SectionHeader>
          )} */}
...

			{
				Dialog && (
					...
					>
						<SkyTypography>
							Xác nhận lưu lịch công tác. Sau khi lưu thông tin sẽ được gửi đến
							người tham gia
						</SkyTypography>
					</CustomDialog>
				)
...
export default withSharedComponents(UpdateTravelWorkSchedules);
