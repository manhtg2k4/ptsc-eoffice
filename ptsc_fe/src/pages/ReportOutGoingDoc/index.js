...
import {
  API_EXPORT_FILE_REPORT,
  API_GET_SOURCE_DOCUMENT,
...
import { listTableSelectOptions } from "./constantReportOutGoingDoc";

const ReportOutGoingDoc = () => {
  ...
  const fetchDataTable = useCallback(
    async (params) => {
      try {
        const selectedOption = listTableSelectOptions.find(
          (opt) => opt.value === selectedTable
        );
        const apiUrl = selectedOption?.api || API_GET_SOURCE_DOCUMENT;
        ...
        let data = raw?.items ?? raw?.data ?? (Array.isArray(raw) ? raw : []);
        if (!Array.isArray(data)) data = [];
        const total = raw?.total ?? raw?.data?.total ?? data.length;
        return { data, total };
      } catch (error) {
...
export default ReportOutGoingDoc;
