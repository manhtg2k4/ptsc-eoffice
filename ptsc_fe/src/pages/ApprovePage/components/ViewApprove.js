...
import { API_NEWS_MANAGEMENT, APP_BASE, API_GET_LIST_UNIT, API_UPLOAD_FILESS } from "@EnvironmentFile/constants/urlConfig";
...
function ViewApprove({ open, onClose, onSuccess, sharedComponents, newsId }) {
  const {
  ...
  const [, setApprovedAt] = useState(null);
  const [showSubmittedAt, setShowSubmittedAt] = useState(false);
  ...
  // Hàm lấy chi tiết tin tức - tách riêng để có thể gọi lại
  ...
  // Hàm submit cập nhật thông tin tin tức đã xuất bản
  ...
  const renderCommentSwitchDialog = useCallback(
    ({ field }) => (
      <FormControlLabel
        control={
          <Checkbox
  ...
  const renderImportantRadioDialog = useCallback(
    ({ field }) => (
      <FormControlLabel
        control={
          <Checkbox
  ...
  }, [isSubmitting, currentUserWorkItem, newsId, toast, onSuccess, getNewsDetail, handleCloseApproveModal, getValues]);
  ...
  // Handler thay đổi recall reason
  ...
  //     const titleMap = {
  //       views: "Danh sách người xem",
  //       likes: "Danh sách người thích",
  //       feedbackNews: "Danh sách người góp ý"
  //     };
  //     const title = titleMap[type] || "Danh sách";
  //     const endpoint = type === "views" ? "viewers" : "likes";
    ...
    <>
        ...
        <FieldContainer container spacing={2}>
          <CheckboxGridItem item xs={6} sm={6}>
            <Controller
              name="isComment"
              control={control}
              render={renderCommentSwitchDialog}
            />
          </CheckboxGridItem>
          <CheckboxGridItem item xs={6} sm={6}>
            <Controller
              name="isImportant"
              control={control}
              render={renderImportantRadioDialog}
            />
          </CheckboxGridItem>
        </FieldContainer>
      </CustomDialog>
      ...
      >
        ...
        <LoadingDialog open={isSubmitting}>
          <StyledDialogContent>
            {openReturnModal && "Đang trả lại tin tức, vui lòng chờ trong giây lát..."}
            {openCancelModal && "Đang hủy tin tức, vui lòng chờ trong giây lát..."}
            {openRecallModal && "Đang thu hồi tin tức, vui lòng chờ trong giây lát..."}
            {!openReturnModal && !openCancelModal && !openRecallModal && "Đang xử lý, vui lòng chờ trong giây lát..."}
          </StyledDialogContent>
        </LoadingDialog>
...
export default withSharedComponents(ViewApprove);