...
const statusDriverOptions = crmSource.find((item) => item.code === "TRANGTHAITAIXE")?.data || [];
  ...
  const onSubmit = useCallback(async (formData) => {
    ...
    try {
      ...
      // Handle image uploads if any new images were added
      const newImages = driverImages.filter(img => img.file);
      if (newImages.length > 0) {
        for (const img of newImages) {
          const fd = new FormData();
          fd.append("file", img.file);
          fd.append("object_type", "listDrivers");
      ...
      toast("Cập nhật thông tin tài xế thành công!", "success");
      onSuccess?.();
      onClose();
    } catch (error) {
  ...
  }, [id, onSuccess, onClose, toast, driverImages]);
  ...
  const handleDeleteFile = useCallback(async () => {
    setIsLoading(true);
    try {
      // Check if the file is already on the server (has no 'file' property which is only for new uploads)
      const fileToDelete = driverImages.find(img => img.id === selectedFileId);
  ...
  }, [selectedFileId, driverImages, toast]);
...
export default withSharedComponents(UpdateDrivers);