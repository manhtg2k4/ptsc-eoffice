...
const ViewRequestCoordination = ({
 ...
 const isView = true;
  ...
  const [currentCoordinationItems, setCurrentCoordinationItems] = React.useState([]); // State for the board
  const [oldCoordination, setOldCoordination] = React.useState(null);
  const [newCoordination, setNewCoordination] = React.useState(null);
...
//   const [selectedItem, setSelectedItem] = React.useState(null);
  ...
  // --- Real State for Coordination ---

  
  // Extract workItem and actionCode if available
  const workItem = data?.workItem || data?.data?.workItem || {};
  const reCoordAction = data?.availableActions?.find(a => 
      a.type === "recoordinate_vehicle_registrant" || 
      a.type === "agree_vehicle_registrant_again"
    ) || data?.availableActions?.[0] || {};
  const actionCode = reCoordAction.code || "";
  ...
  const availableVehicles = carsList;
  ...
  const handleSelectCar = React.useCallback((event) => {
    const id = event.currentTarget.getAttribute("data-id");
    const car = availableVehicles.find(v => v.id === id);
    if (car && oldCoordination) {
      setNewCoordination({
        plate: car.licensePlate,
        type: car.carType,
        brand: car.brand,
        driver: car.manager?.name || "—",
        carId: car.id,
        driverId: car.manager?.id || car.manager_id || null,
        carObj: car,
        driverObj: car.manager ? { id: car.manager.id || car.manager_id, "full_name": car.manager.name } : null
      });

      // Update the main selected list for the confirm dialog

    }
  }, [availableVehicles, oldCoordination]);

  const handleSelectDriver = React.useCallback((event) => {
    const id = event.currentTarget.getAttribute("data-id");
    const driver = availableDrivers.find(d => d.id === id);
    if (driver && oldCoordination) {
      setNewCoordination(prev => ({
        ...(prev || oldCoordination),
        driver: driver.fullName || driver.full_name || driver.name,
  ...
  const handleApplyNewCoordination = React.useCallback(() => {
    ...
    setCurrentCoordinationItems(prev => prev.map(item => {
      // Find the row that matches the oldCoordination.id (which is the carId)
      if (item.carId === oldCoordination.id) {
        return {
          ...item,
          carId: newCoordination.carId || item.carId,
          brand: newCoordination.carObj?.brand || item.brand,
          licensePlate: newCoordination.carObj?.licensePlate || item.licensePlate,
          carType: newCoordination.carObj?.carType || item.carType,
          driverName: newCoordination.driverObj?.fullName || newCoordination.driverObj?.full_name || newCoordination.driver || item.driverName,
          car: newCoordination.carObj ? {
             id: newCoordination.carObj.id,
             licensePlate: newCoordination.carObj.licensePlate,
             carType: newCoordination.carObj.carType,
             brand: newCoordination.carObj.brand,
             seatCount: newCoordination.carObj.seatCount || newCoordination.carObj.capacity
          } : item.car,
          driverId: newCoordination.driverId || item.driverId,
          driver: newCoordination.driverObj ? {
             id: newCoordination.driverObj.id,
             driverId: newCoordination.driverObj.driverId || newCoordination.driverId,
             fullName: newCoordination.driverObj.fullName || newCoordination.driverObj.full_name || newCoordination.driver
          } : (newCoordination.driver ? {
             id: newCoordination.driverId,
             driverId: newCoordination.driverId,
             fullName: newCoordination.driver
          } : item.driver)
        };
      }
      return item;
    ...
    // Reset top boxes
    setOldCoordination(null);
    setNewCoordination(null);
  }, [oldCoordination, newCoordination]);

  const handlePrepareRecoordinate = React.useCallback((event) => {
    const id = event.currentTarget.getAttribute("data-id");
    const item = currentCoordinationItems.find(i => (i.carId || i.id) === id);
    if (!item) return;

    setOldCoordination({
      id: item.carId, // Use carId as unique key for re-coordination
      plate: item?.licensePlate || item.carPlate || item.plate || "—",
      type: item.car?.carType || item.carType || item.capacity || "—",
      brand: item?.brand || item.carBrand || "—",
      driver: item.driver?.fullName || item.driverName || item.driver || "—",
    });
    setNewCoordination(null);
  }, [currentCoordinationItems]);
  ...
  const coordinationItems = currentCoordinationItems; // Use stateful list
  ...
  const currentSeatCount = coordinationItems.reduce((sum, item) => sum + getSeatCount(item), 0);

  const coordinationDataComputed = {
    status: currentSeatCount >= currentDemand ? "Đã điều phối đủ" : "Chưa điều phối đủ",
    demand: currentDemand,
    vehicleCount: coordinationItems.length,
      ...
      if (open && requestId) {
        ...
        try {
          ...
          if (response && response.success) {
            ...
            if (vehicleData.coordinationInformation) {
              setCurrentCoordinationItems(vehicleData.coordinationInformation);
            }
          }
        } catch (error) {
...
<path d="M14 14.5C14 14.6326 13.9473 14.7598 13.8535 14.8535C13.7598 14.9473 13.6326 15 13.5 15H6.99998C6.86737 15 6.7402 14.9473 6.64643 14.8535C6.55266 14.7598 6.49998 14.6326 6.49998 14.5C6.49998 14.3674 6.55266 14.2402 6.64643 14.1464C6.7402 14.0527 6.86737 14 6.99998 14H13.5C13.6326 14 13.7598 14.0527 13.8535 14.1464C13.9473 14.2402 14 14.3674 14 14.5ZM13 8.99999H8.98623L6.99998 4.99999L7.88686 3.35499C7.88952 3.35061 7.89182 3.34601 7.89373 3.34124C8.01224 3.10411 8.03174 2.82962 7.94794 2.57811C7.86415 2.32661 7.68391 2.11866 7.44686 1.99999L7.41748 1.98624L5.31248 1.09812C5.07612 0.985236 4.80491 0.969596 4.55714 1.05456C4.30937 1.13953 4.10483 1.31832 3.98748 1.55249L2.60498 4.30249C2.53593 4.44156 2.5 4.59473 2.5 4.74999C2.5 4.90526 2.53593 5.05843 2.60498 5.19749L6.23686 12.4475C6.31959 12.614 6.44727 12.7539 6.60544 12.8516C6.76362 12.9492 6.94597 13.0006 7.13186 13H13C13.2652 13 13.5196 12.8946 13.7071 12.7071C13.8946 12.5196 14 12.2652 14 12V9.99999C14 9.73478 13.8946 9.48042 13.7071 9.29289C13.5196 9.10535 13.2652 8.99999 13 8.99999Z" fill="black"/>
                ...
                <tbody>
                  {coordinationDataComputed.items.map((item, index) => (
                    <tr key={item.carId || index}>
                      <td>{index + 1}</td>
                      <td>{item.brand || item.car?.brand || "-"}</td>
                      <td>{item.licensePlate || item.car?.licensePlate || "-"}</td>
              ...
              <SummaryBoxFlex>
                <ReCoordinationBox>
                  <ReCoordinationTitle>ĐIỀU PHỐI CŨ</ReCoordinationTitle>
                    <ReCoordinationRow>
                      ...
                      <ReCoordinationValue>{displayOldCoordination.plate}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Loại xe:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayOldCoordination.type}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Hãng xe:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayOldCoordination.brand}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Tài xế:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayOldCoordination.driver}</ReCoordinationValue>
                    </ReCoordinationRow>
                </ReCoordinationBox>
                ...
                <ReCoordinationBox>
                  <ReCoordinationTitle>ĐIỀU PHỐI MỚI</ReCoordinationTitle>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Biển số xe:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayNewCoordination.plate}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Loại xe:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayNewCoordination.type}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Hãng xe:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayNewCoordination.brand}</ReCoordinationValue>
                    </ReCoordinationRow>
                    <ReCoordinationRow>
                      <ReCoordinationLabel>Tài xế:</ReCoordinationLabel>
                      <ReCoordinationValue>{displayNewCoordination.driver}</ReCoordinationValue>
                    </ReCoordinationRow>
                </ReCoordinationBox>
              </SummaryBoxFlex>
            </StyledBoxContainerContent>
      ...
      <ConfirmRecoordinationDialog 
        ...
        // noteDetail={reason}
        formValues={{
          requestType: useWatch({ control, name: "requestType" }),
          priority: useWatch({ control, name: "priority" }),
          isImportantGuest: useWatch({ control, name: "isImportantGuest" }),
          departureTime: useWatch({ control, name: "departureTime" }),
          returnTime: useWatch({ control, name: "returnTime" }),
          departurePoint: useWatch({ control, name: "departurePoint" }),
          destination: useWatch({ control, name: "destination" }),
          passengerCount: useWatch({ control, name: "passengerCount" }),
        }}
...
export default withSharedComponents(ViewRequestCoordination);