 ...
 */
const ConfirmCoordinateRequestsDialog = ({
  open,
  onClose,
  onSuccess,
  formValues = {},
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      const coordinationInformation = selectedCars.map((car) => ({
        carId: car.id,
        driverId: selectedDriverMap[car.id]?.driverId || selectedDriverMap[car.id]?.id || car.manager?.id || null,
      }));
...
export default ConfirmCoordinateRequestsDialog;
