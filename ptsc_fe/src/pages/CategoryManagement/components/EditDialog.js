...
const EditDialog = ({
 ...
 const handleSave = async () => {
    const [isTableValid] = await Promise.all([
      trigger(["code", "title"]),
      // trigger(["code", "title", "moduleCategory"]),
      dynamicTableRef.current?.validate(),
    ]);
 

    if (!isTableValid) {
      toast("Vui lòng kiểm tra lại các giá trị trong bảng.", "error");
      return;
    }
    ...
    // Clear newAddedIds khi save thành công
    // dynamicTableRef.current?.clearNewAddedIds();
    onSave(payload);
  };
		...
		<CustomSwipper
			...
			footer={
				<>
					<FlexGrowBox />
					<FooterActions>
						<ButtonOutline variant="outlined" onClick={handleSave}>
							Lưu
						</ButtonOutline>
					</FooterActions>
				</>
		...
		>
					...
					<Grid container spacing={2}>
						<Grid item xs={12} md={4} sm={6}>
							<Controller
								...
								render={({ field }) => (
									<>
										<StyledFormLabel>Mã danh mục <span style={{ color: "red", marginLeft: "2px" }}>*</span></StyledFormLabel>
										<CustomInput
											{...field}
											error={!!errors.code}
						...
						<Grid item xs={12} md={4} sm={6}>
							<Controller
								...
								render={({ field }) => (
									<>
										<StyledFormLabel>Tên danh mục <span style={{ color: "red", marginLeft: "2px" }}>*</span></StyledFormLabel>
										<CustomInput
											{...field}
											error={!!errors.title}
							...
							<Controller
								...
								render={({ field }) => (
									<WapperCustomAutoCompleteSearch
										...
										error={!!errors.moduleCategory}
										helperText={errors.moduleCategory?.message}
										unsetFontWeight
										isMulti
									/>
								)}
            ...
            />
				...
				<StyledBoxContainerContent>
					...
					<StyledDivider /> 
					<DynamicValuesTable ref={dynamicTableRef} defaultValue={defaultData?.data} type="edit" idDocumentParent={idDocumentParent} customMaxHeight={600}/>
				</StyledBoxContainerContent>
			</FormContainer>
		</CustomSwipper>
	);
