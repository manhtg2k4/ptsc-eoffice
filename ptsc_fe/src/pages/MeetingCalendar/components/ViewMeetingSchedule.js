...
const DelegationDialog = styled(Dialog)(() => ({
  '& .MuiDialog-paper': {
    borderRadius: '16px',
    width: '600px',
    maxWidth: '90%',
...
const DelegationHeader = styled(Box)(({ theme }) => ({
  display: 'flex',
  alignItems: 'center',
...
const DelegationTitleBox = styled(Box)(() => ({
  display: 'flex',
  alignItems: 'center',
  gap: '12px',
}));

const DelegationIconWrapper = styled(Box)(() => ({
  width: '36px',
  height: '36px',
  borderRadius: '8px',
...
const DelegationContent = styled(DialogContent)(({ theme }) => ({
...
const DelegationCard = styled(Box)(({ theme }) => ({
...
const DelegationCardHeader = styled(Box)(() => ({
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'space-between',
}));
...
const StatusDot = styled(Box)(() => ({
  ...
  height: '8px',
  borderRadius: '50%',
  backgroundColor: '#B25E00',
}));

const RequestIdText = styled(Typography)(({ theme }) => ({
  fontSize: '13px',
  color: theme.palette.text.secondary,
}));

const DelegationFlowBox = styled(Box)(() => ({
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'space-between',
  gap: '16px',
...
const DelegationPersonBox = styled(Box)(() => ({
  display: 'flex',
  flexDirection: 'column',
  alignItems: 'center',
  textAlign: 'center',
...
const FlowArrow = styled(ArrowForwardIcon)(() => ({
  color: '#94A3B8',
  fontSize: '24px',
}));

const DelegationCardActions = styled(Box)(({ theme }) => ({
  display: 'flex',
...
const DelegationFooter = styled(DialogActions)(({ theme }) => ({
  padding: theme.spacing(2, 3),
...
const EmptyDelegationText = styled(Typography)(({ theme }) => ({
  color: theme.palette.text.secondary,
  textAlign: 'center',
...
const FooterApproveAllButton = styled(Button)(() => ({
  backgroundColor: '#0066CC',
  color: '#FFFFFF',
  borderRadius: '8px',
  textTransform: 'none',
...
const DelegationCardItem = memo(({ item, onApprove, onReject }) => {
  ...
  return (
    <DelegationCard>
      <DelegationCardHeader>
        ...
        <RequestIdText>{`Yêu cầu #${item.id || 'N/A'}`}</RequestIdText>
      </DelegationCardHeader>

      <DelegationFlowBox>
        <DelegationPersonBox>
          <PersonAvatar src="" alt={item.userName}>
            {getInitials(item.userName)}
          </PersonAvatar>
          <PersonName>{item.userName || "Người ủy quyền"}</PersonName>
          <PersonTitle>
            {item.position && item.unitName ? `${item.position} - ${item.unitName}` : (item.position || item.unitName || "---")}
          </PersonTitle>
        </DelegationPersonBox>
        ...
        <DelegationPersonBox>
          <PersonAvatar src="" alt={item.delegatedToUserName}>
            {getInitials(item.delegatedToUserName)}
          </PersonAvatar>
          <PersonName>{item.delegatedToUserName || "Người được ủy quyền"}</PersonName>
          <PersonTitle>
            {item.delegatedToPosition && item.delegatedToUnitName ? `${item.delegatedToPosition} - ${item.delegatedToUnitName}` : (item.delegatedToPosition || item.delegatedToUnitName || "---")}
          </PersonTitle>
        </DelegationPersonBox>
      </DelegationFlowBox>

      <DelegationCardActions>
        <RejectButton variant="outlined" onClick={handleRejectClick}>
          Từ chối
        </RejectButton>
        <ApproveButton variant="contained" onClick={handleApproveClick}>
          Phê duyệt
        </ApproveButton>
      </DelegationCardActions>
    </DelegationCard>
...
DelegationCardItem.displayName = "DelegationCardItem";
...
const ProcessingStatusBadge = ({ status }) => {
  const items = [
    ...
    (status?.hasDocumentUser == false ) ? null : { key: 'prepareDocumentUser', label: "Chuẩn bị tài liệu họp" },
  ].filter(Boolean);
...
const MemberItem = memo(({ member, noStatus }) => {
...
const FileRow = memo(({ fileItem, fileIndex, taskId, onToggleImportant, onRemoveFile }) => {
...
const ViewMeetingSchedule = ({
 ...
 const isView = true;
  ...
  const [openPrepareDocs, setOpenPrepareDocs] = useState(false);
  const [docAssignee, setDocAssignee] = useState(null);
  ...
  const [canAccessMeetingTabs, setCanAccessMeetingTabs] = useState(true);
  ...
  const [meetingTasksListData, setMeetingTasksListData] = useState([]);
  const [isTasksListLoading, setIsTasksListLoading] = useState(false);
  ...
  const [openHandleMeetingSchedules, setOpenHandleMeetingSchedules] = useState(false);
  const [personalMeetingActionCode, setPersonalMeetingActionCode] = useState("");
  const [openViewJob, setOpenViewJob] = useState(false);
  const [viewingJob, setViewingJob] = useState(null);
  const [openDelegationModal, setOpenDelegationModal] = useState(false);

  const handleOpenDelegationModal = useCallback(() => {
    setOpenDelegationModal(true);
  }, []);

  const handleCloseDelegationModal = useCallback(() => {
  ...
  const meetingConclusionRef = useRef(null);
  ...
  }, [selectedUnits,meetingData, guests]);

  const chairman = useMemo(() => selectedUnits.find(u => u.roles?.chair), [selectedUnits]);
  const secretary = useMemo(() => selectedUnits.find(u => u.roles?.secretary), [selectedUnits]);
...
setTotalParticipants(data?.totalParticipants || 0);
   ...
   const fetchMeetingTasksList = useCallback(async () => {
    if (open && meetingId) {
      ...
      try {
        ...
        // ❌ Removed: fetchMeetingDetails() was incorrectly called here, causing duplicate API calls on mount.
        // The meeting details are already fetched independently via its own useEffect.
      } catch (error) {
  ...
  const [delegationActionLoading, setDelegationActionLoading] = useState(null);

  const fetchPendingDelegations = useCallback(async () => {
    if (!meetingId) return [];
    try {
      const response = await axiosInstance.get(`${API_ADD_MEETING_SCHEDULE}/${meetingId}/pending-delegations`);
      const data = response?.data?.data || response?.data || response || [];
      const list = Array.isArray(data) ? data : [];
      setDelegationRequests(list);
      return list;
    } catch (error) {
      toast(error?.response?.data?.message || "Lỗi khi tải danh sách ủy quyền", "error");
      return [];
    }
  }, [meetingId, toast]);

  useEffect(() => {
    if (openDelegationModal) {
      fetchPendingDelegations();
    }
  }, [openDelegationModal, fetchPendingDelegations]);

  const handleApproveDelegation = useCallback(async (item) => {
    try {
      setDelegationActionLoading({ id: item.id, type: "APPROVE" });
      setIsLoading(true);
      const payload = {
        participantIds: [item.id],
        action: "APPROVE"
      };
      await axiosInstance.post(`${API_ADD_MEETING_SCHEDULE}/${meetingId}/delegation/resolve`, payload);
      toast(`Đã phê duyệt yêu cầu ủy quyền của ${item.userName}`, "success");
      const updatedDelegations = await fetchPendingDelegations();
      await fetchMeetingDetails();
      await fetchParticipants();
      if (!updatedDelegations || updatedDelegations.length === 0) {
        setOpenDelegationModal(false);
      }
    } catch (error) {
      toast(error?.response?.data?.message || "Lỗi khi phê duyệt yêu cầu ủy quyền", "error");
    } finally {
      setIsLoading(false);
      setDelegationActionLoading(null);
    }
  }, [meetingId, fetchPendingDelegations, fetchMeetingDetails, fetchParticipants, toast, setOpenDelegationModal]);

  const handleRejectDelegation = useCallback(async (item) => {
    try {
      setDelegationActionLoading({ id: item.id, type: "REJECT" });
      setIsLoading(true);
      const payload = {
        participantIds: [item.id],
        action: "REJECT"
      ...
      await axiosInstance.post(`${API_ADD_MEETING_SCHEDULE}/${meetingId}/delegation/resolve`, payload);
      toast(`Đã từ chối yêu cầu ủy quyền của ${item.userName}`, "info");
      const updatedDelegations = await fetchPendingDelegations();
      await fetchMeetingDetails();
      await fetchParticipants();
      if (!updatedDelegations || updatedDelegations.length === 0) {
      ...
      toast(error?.response?.data?.message || "Lỗi khi từ chối yêu cầu ủy quyền", "error");
    } finally {
      setIsLoading(false);
      setDelegationActionLoading(null);
    }
  ...
  const handleApproveAllDelegations = useCallback(async () => {
    if (delegationRequests.length === 0) return;
    try {
      setDelegationActionLoading({ id: "ALL", type: "APPROVE_ALL" });
      setIsLoading(true);
      const payload = {
        participantIds: delegationRequests.map(req => req.id),
        action: "APPROVE"
      };
      await axiosInstance.post(`${API_ADD_MEETING_SCHEDULE}/${meetingId}/delegation/resolve`, payload);
      toast("Đã phê duyệt toàn bộ yêu cầu ủy quyền!", "success");
      await fetchPendingDelegations();
      await fetchMeetingDetails();
      await fetchParticipants();
      setOpenDelegationModal(false);
    } catch (error) {
      toast(error?.response?.data?.message || "Lỗi khi phê duyệt toàn bộ yêu cầu ủy quyền", "error");
    } finally {
      setIsLoading(false);
      setDelegationActionLoading(null);
    }
  }, [meetingId, delegationRequests, fetchPendingDelegations, fetchMeetingDetails, fetchParticipants, toast, setOpenDelegationModal]);
  ...
  }, [meetingId, fetchParticipants, fetchMeetingDetails, toast, handleCloseLockConfirm]);
  ...
  const [openUpdateMeeting, setOpenUpdateMeeting] = useState(false);
    ...
    <BaseSwipper
      ...
      moreActions={
        <StyledGridActions container spacing={1}>
          {((meetingData?.isParticipant || meetingData?.isChairman || meetingData?.isSecretary) && !meetingData?.isRejectUser)&& !meetingData.isDelegating && (
          ...
          {activeTabId === 'info' && (
            <>
              {availableActions.some(action => action.type === 'approve_delegate') && (
                <Grid item>
                  <ButtonOutline
                    ...
                    variant="outlined"
                  >
                    {availableActions.find(action => action.type === 'approve_delegate')?.actionName || availableActions.find(action => action.type === 'approve_delegate')?.label || 'ỦY QUYỀN'}
                  </ButtonOutline>
                </Grid>
              )}
              {availableActions.some(action => action.type === 'cancel_join_meeting') && (
                <Grid item>
                  <ButtonOutline
                    onClick={handleOpenCancelJoinConfirm}
                    disabled={isLoading}
                    variant="outlined"
                  >
                    HỦY THAM GIA
...
{/*                           
      ...
      <CustomDialog
        open={openCancelJoinConfirm}
        onClose={handleCloseCancelJoinConfirm}
        onSave={handleCancelJoinMeeting}
      ...
      >
        <SkyBox>
          <SkyTypography>
            Bạn có chắc chắn muốn hủy tham gia cuộc họp này không?
          </SkyTypography>
        </SkyBox>
      </CustomDialog>
      ...
      >
        <DelegationHeader>
          <DelegationTitleBox>
            <DelegationIconWrapper>
              <CompareArrowsIcon />
            </DelegationIconWrapper>
            <DelegationTitle variant="h6">
              Danh sách ủy quyền họp
            </DelegationTitle>
          </DelegationTitleBox>
          <CloseIconButton onClick={handleCloseDelegationModal} size="small">
            <CloseIcon />
          </CloseIconButton>
        </DelegationHeader>

        <DelegationContent dividers>
          {delegationRequests.length > 0 ? (
            delegationRequests.map((item, idx) => (
              <DelegationCardItem
                key={item.id || item.participantId || idx}
                item={item}
            ...
            <EmptyDelegationText variant="body2">
              Không có yêu cầu ủy quyền nào cho cuộc họp này.
            </EmptyDelegationText>
          )}
        </DelegationContent>
      ...
      <LoadingDialog open={!!delegationActionLoading}>
        {delegationActionLoading?.type === "APPROVE"
          ? "Đang phê duyệt yêu cầu ủy quyền, vui lòng chờ trong giây lát..."
          : delegationActionLoading?.type === "REJECT"
          ? "Đang từ chối yêu cầu ủy quyền, vui lòng chờ trong giây lát..."
          : delegationActionLoading?.type === "APPROVE_ALL"
          ? "Đang phê duyệt tất cả yêu cầu ủy quyền, vui lòng chờ trong giây lát..."
...
export default withSharedComponents(ViewMeetingSchedule);
