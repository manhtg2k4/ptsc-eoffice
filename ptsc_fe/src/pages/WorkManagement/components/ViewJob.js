...
import { DeleteOutline, DownloadOutlined, FolderOutlined, VisibilityOutlined } from "@mui/icons-material";
...
import { addCommentToJob, deleteCommentInJob, getCommentsByTask, replyToCommentInJob, toggleCommentLike, updateCommentInJob } from "@redux/slices/SharedCategory/managementUnitSlice";
import api from "@services/api";
import { apiUploadFile } from "@services/FileUpload/fileUpload";
import { SkyBox, SkyCheckbox, SkyFlexGap8, SkyGrid, SkyTypography, SkyTooltip } from "@styles/SkyStyles";
import axiosInstance from "@utils/axiosInstance";
import DOMPurify from "dompurify";
import { useDispatch, useSelector } from "react-redux";
import AddJobChild from "./AddJobChild";
...
import UpdateJobDialogNew from "./UpdateJobDialogNew";
import ViewJobChild from "./ViewJobChild";
...
const ViewJob = ({
  ...
  onSuccess,
  sharedComponents,
  title = "Chi tiết công việc chung",
  ...
  // confirmAddSubJob đã được chuyển sang AddJobChild
    ...
    if (open && currentTaskId) {
      ...
      try {
        ...
        const formData = {
          ...
          isConfidential: !!jobDetail.isConfidential,
        };
        reset(formData);
  ...
  const subTaskColumns = [
    {
      ...
      accessor: (row) => {
        return (
          <SkyFlexGap8>
            <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(row?.flag) }} />
  ...
  const childBreadcrumbs = useMemo(() => {
    ...
    return [...baseBreadcrumbs, { title: title || "Chi tiết công việc chung", path: "CLOSE_SWIPER" }];
  ...
  return (
    ...
    >
      <JobMainContent>
          ...
          <Grid container spacing={2} mb={3}>
            <Grid item xs={12} md={8} lg={9.2}>
              <StyledBoxContainerContentHeader styledMargin="left" fullHeight>
                ...
                {/* Tên công việc */}
                <SkyTooltip title={watch("taskName") || ""} arrow placement="top">
        ...
        <StyledBoxContainerContent styledMarginTop>
          <JobSectionHeader mt={2}>
            <SkyFlexGap8>
              ...
              <ButtonOutline onClick={showMore}>
                {showMoreGeneral ? "Thu gọn" : "Xem thêm"}
              </ButtonOutline>

            </SkyFlexGap8>
            {flags?.canUpdate && <JobActionOutlineButton startIcon={<EditIcon />} onClick={handleOpenUpdateGeneralDialog}>Chỉnh sửa</JobActionOutlineButton>}
          </JobSectionHeader>
          <StyleLine />
          <Grid container rowSpacing="6px" columnSpacing="10px" mb={1}>
            ...
            {showMoreGeneral && (
              <>
                ...
                {typeJob !== 'noTatic' && parentName ? null : (
                  <Grid item xs={12} md={4}>
                    <Controller
                      name="topic"
                      control={control}
                ...
                <Grid item xs={12} md={4}>
                  {/* Checkbox Placeholder to match Image 1 */}
                  <ParticipantInfoContainer>
                    <Controller
                ...
                <Grid container rowSpacing={3} columnSpacing={3}>
                  ...
                  {/* {!(displayData?.createdBy === nameUser && assignerUsername === nameUser) && (
                    <Grid item xs={12} md={6}>
                      <WrappedCustomAsyncAutoComplete
                        limitTags={2}
                        options={displayData?.createdBy ? [displayData.createdBy] : []}
                        value={displayData?.createdBy ? [displayData.createdBy] : []}
                        disabled
                        isMulti
                        optionValue="_id"
                        optionLabel="name"
        ...
        {/* ==================== TÀI LIỆU CÔNG VIỆC & KẾT QUẢ ĐẦU RA ==================== */}
      ...
      {/* Dialog cập nhật */}
      {isFromProject && updateDialogState.type === 'participants' ? (
        <UpdateJobDialogNew
          ...
          parentType={subTasksData?.parentType || parentType}
          parentName={displayData?.parentName}
          isFromProject={isFromProject}
          projectId={projectId || displayData?.projectId || data?.projectId || data?.project?.id}
          projectDetail={projectDetail || data?.project}
          dataDetail={displayData}
          data={
      ...
      <ViewJobChild
        ...
        breadcrumbs={childBreadcrumbs}
        startDateParent={displayData?.deadlineStartParentISO ?? displayData?.deadlineStartParent ?? startDateParent ?? data?.deadlineStartParentISO ?? data?.deadlineStartParent}
        endDateParent={displayData?.deadlineEndParentISO ?? displayData?.deadlineEndParent ?? endDateParent ?? data?.deadlineEndParentISO ?? data?.deadlineEndParent}
