...
import UpdateJobToDocumentDialog from "./UpdateJobToDocumentDialog";
import AddNewJob from "./AddNewJob";
import FileTreeTable from "@components/FileTreeTable";
import axiosInstance from "@utils/axiosInstance";
import { API_ADD_COMMON_WORK, APP_BASE, API_GET_COMMON_WORK_DETAIL, API_GET_SUBTASKS, API_JOB_TO_DOCUMENT, API_COMMON_WORK_COMMENTS, API_XLSX_TO_PDF, API_MERGE_LINK } from "@EnvironmentFile/constants/urlConfig";
...
import withFormWrapper, { ViewFieldBox, ViewFieldLabel, ViewFieldValue } from "@components/common/FormWrapper";

const StyledDescriptionIcon = styled(DescriptionIcon)(() => ({
  color: "#0062AD",
}));

const ViewJobToDocument = ({
  open,
  onClose,
  onSuccess,
  sharedComponents,
  ...
  const summary = data?.summary || displayData?.summary;
  const toBook = data?.toBook || displayData?.toBook;
  const fromBook = data?.documentDate || displayData?.documentDate;
  const documentType = data?.documentType || displayData?.documentType;
  ...
  const handleOpenDialogDocument = () => {
    setViewDocumentId(data?.docId || documentId);
  ...
  const handleFileUpload = useCallback(async (event, objectType) => {
    ...
    await processFilesUpload(files, objectType); fetchHistory?.()
    event.target.value = null;
  }, [processFilesUpload, fetchHistory]);
  ...
  const handleDragOverTask = useCallback((e) => {
    if (flags?.canUpdateFromDoc) handleDragOver(e, setIsDraggingTask);
  }, [flags?.canUpdateFromDoc, handleDragOver]);
  ...
  const handleDropTask = useCallback((e) => {
    if (flags?.canUpdateFromDoc) handleDrop(e, "taskdocuments", setIsDraggingTask);
  }, [flags?.canUpdateFromDoc, handleDrop]);

  const handleDragOverFinal = useCallback((e) => {
    if (flags?.canUpdateFolderFromDoc) handleDragOver(e, setIsDraggingFinal);
  }, [flags?.canUpdateFolderFromDoc, handleDragOver]);
  ...
  const handleDropFinal = useCallback((e) => {
    if (flags?.canUpdateFolderFromDoc) handleDrop(e, "finaldocuments", setIsDraggingFinal);
  }, [flags?.canUpdateFolderFromDoc, handleDrop]);
  ...
  const fetchJobDetail = useCallback(async () => {
    if (open && currentTaskId) {
      ...
      try {
        setIsLoading(true);
        const response = await axiosInstance.get(`${API_JOB_TO_DOCUMENT}/${currentTaskId}`);
        ...
        const formData = {
          ...
          toBook: jobDetail.toBook,
          documentDate: jobDetail.documentDate ? dayjs(jobDetail.documentDate) : null,
          documentType: jobDetail.documentType,
          note: jobDetail.note,
  ...
  const childBreadcrumbs = useMemo(() => {
    ...
    return [...baseBreadcrumbs, { title: title || "Chi tiết công việc từ văn bản", path: "CLOSE_SWIPER" }];
    ...
    >
      <JobMainContent>
        ...
        <StyledBoxContainerContent styledMarginTop>
          ...
          ) : (summary || toBook || fromBook || documentType) ? (
            <Grid container rowSpacing="6px" columnSpacing="10px" mb={1}>
              {/* Nguồn văn bản - Full width */}
              <Grid item xs={12}>
                    ...
                    <Controller
                      ...
                      render={({ field }) => {
                        if (isViewMode) {
                          return (
                            <ViewFieldBox
                              onClick={handleOpenDialogDocument}
                              style={{ cursor: "pointer" }}
                            >
                              <ViewFieldLabel>Nguồn văn bản</ViewFieldLabel>
                              <ViewFieldValue
                                component="div"
                                style={{
                                  color: "#0062AD",
                              ...
                              >
                                {field.value ? (
                                  <>
                                    <StyledDescriptionIcon style={{ fontSize: "20px" }} />
              ...
              {/* Số văn bản + Tên công việc + Ngày trả văn bản in same row */}
              ...
              {/* Row 3: Tên công việc (md=8), Ngày bắt đầu - Hạn xử lý (md=4) */}
              ...
              {/* Row 4: Mã công việc, Độ ưu tiên, Quy trình */}
              <Grid item xs={12} md={4}>
                <Controller
                  name="code"
                  control={control}
              ...
              {/* Row 5: Thời gian nhắc hạn, Chủ đề, Công việc cần phê duyệt */}
              <Grid item xs={12} md={4}>
                <Controller
                  name="reminderTime"
                  control={control}
            ...
            <Grid container rowSpacing="6px" columnSpacing="10px" mb={1}>
              ...
              <Grid item xs={12} lg={4}>
                <Grid container rowSpacing="6px" columnSpacing="10px" >
                  ...
                  {/* Các trường lặp lại sẽ hiển thị ở đây nếu có */}
                  {/* {repeatTask && (...)} */}
                </Grid>
              </Grid>
            </Grid>
        ...
        <Grid container spacing={3} mt={0.5}>
          <DocumentsGridItem item xs={12} md={6}>
            ...
            <UploadDropZone
              ...
              disabled={!flags?.canUpdateFromDoc}
            ...
            >
              ...
              <FileTreeTable
                ...
                emptyMessage={flags?.canUpdateFromDoc ? "Kéo thả tệp hoặc thư mục vào đây để tải lên" : "Chưa có tài liệu nào"}
              />
            </UploadDropZone>
          </DocumentsGridItem>
          <DocumentsGridItem item xs={12} md={6}>
            ...
            <UploadDropZone
              ...
              disabled={!flags?.canUpdateFolderFromDoc}
            ...
            >
              ...
              <FileTreeTable
                ...
                sourceAsync
                fileName={displayData?.name || data?.taskName}
                emptyMessage={flags?.canUpdateFolderFromDoc ? "Kéo thả tệp hoặc thư mục vào đây để tải lên" : "Chưa có tài liệu nào"}
              />
            </UploadDropZone>
          </DocumentsGridItem>
        </Grid>
      ...
      >
        ...
        {checkCanDeleteFile && <MenuItem onClick={handleOpenDeleteDialog}>
          <StyledListItemIcon>
            <DeleteOutlined />
          </StyledListItemIcon>
      ...
      <ViewJob
        ...
        parentType={subTasksData?.parentType}
        typeJob='noTatic'
        breadcrumbs={childBreadcrumbs}
...
export default withSharedComponents(ViewJobToDocument);