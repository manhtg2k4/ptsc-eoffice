...
import {
  ...
  SkyPopover as Popover,
  SkyList as List,
  SkyListItem as ListItem,
  SkyFlexGap8,
  SkyCheckbox,
  SkyTypography,
  SkyTooltip,
...
import { VisibilityOutlined, DeleteOutline, DownloadOutlined, FolderOutlined } from "@mui/icons-material";
...
import { API_ADD_COMMON_WORK, APP_BASE, API_GET_COMMON_WORK_DETAIL, API_GET_SUBTASKS, API_XLSX_TO_PDF, API_MERGE_LINK } from "@EnvironmentFile/constants/urlConfig";
...
const ViewJobChild = ({
  ...
  // confirmAddSubJob được chuyển sang AddJobChild
  ...
  const refetchFiles = useCallback(async () => {
    ...
    try {
      const currentUserName = authUser?.user?.name || authUser?.name || "Người dùng";
    ...
    if (isFolderUpload) {
      ...
      for (const file of files) {
        ...
        if (file.size > UPLOAD_LIMITS_PER_FILE.MAX_SIZE) {
          toast(
            `File "${truncateFileName(file.name, 20)}" vượt quá giới hạn ${formatFileSize(UPLOAD_LIMITS_PER_FILE.MAX_SIZE)}. ` +
    ...
    if (open && currentTaskId) {
      ...
      try {
        ...
        const formData = {
          ...
          templateId: jobDetail.templateId,
  ...
  }, [commentText, currentTaskId, dispatch, toast, mentionedIds, editingCommentId, replyingToCommentId, authUser]);
  ...
  //   const Wrapped = withFormWrapper(AutoCompleteAsync, "asyncSelect");
  //   const Component = (props) => <Wrapped {...props} isview />;
                ...
                <JobStatusDatesRow>
                  <JobStatusDateItem>
                    ...
                    <JobStatusDateValue variant="body1">
                      {watch("startDate") ? new Date(watch("startDate")).toLocaleDateString('vi-VN') : "N/A"}
                    </JobStatusDateValue>
                  </JobStatusDateItem>
                  <JobStatusDateItem>
                    <JobStatusDateLabel variant="caption">
                    ...
                    <JobStatusDateValue variant="body1">
                      {watch("deadline") ? new Date(watch("deadline")).toLocaleDateString('vi-VN') : "N/A"}
                    </JobStatusDateValue>
                  </JobStatusDateItem>
                </JobStatusDatesRow>
...
export default withSharedComponents(ViewJobChild);