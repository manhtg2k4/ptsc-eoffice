...
const AddNewJob = ({
  ...
  const schema = yup.object().shape({
    ...
    deadline: yup
      ...
      .test(
        'recurring-task-duration',
        function (value) {
          const { startDate, repeatTask } = this.parent;
          if (!value || !startDate || !repeatTask || repeatTask === 'khong') return true;

          const daysDiff = dayjs(value).diff(dayjs(startDate), 'day');

          if (repeatTask === 'tuan' && daysDiff > 6) {
            return this.createError({ message: 'Thời gian thực hiện công việc lặp hàng tuần tối đa là 6 ngày.' });
          }
          if (repeatTask === 'thang' && daysDiff > 27) {
            return this.createError({ message: 'Thời gian thực hiện công việc lặp hàng tháng tối đa là 27 ngày.' });
          }
          if (repeatTask === 'quy' && daysDiff > 89) {
            return this.createError({ message: 'Thời gian thực hiện công việc lặp hàng quý tối đa là 89 ngày.' });
          }
    ...
    monthInQuarter: yup.string().transform((value) => (Array.isArray(value) ? value[0] || "" : value)).when("repeatTask", {
  ...
  const handleWeekDayChange = useCallback((field, dayValue) => {
  ...
  const handleUploadFiles = useCallback(async (files, taskId, objectType) => {
    ...
    if (isFolderUpload) {
      ...
      for (const file of files) {
        try {
          ...
          const uploadResponse = await apiUploadFile(file, objectType, taskId);
  ...
  const onSubmit = useCallback(async (data, bypassFlag = false) => {
    ...
    const validUploadedFiles = uploadedFiles.filter(f => {
      // Bỏ qua kiểm tra độ dài tên cho link (nó là đối tượng bth ko phải File)
    ...
    const payload = {
      ...
      topic: data.mode || parentTopic || null,
      note: data.description,
      progress: "0",
      parent: parentId ? (isNaN(Number(parentId)) ? parentId : Number(parentId)) : null,
      projectId: projectId,
    ...
    try {
      ...
      const checkApi = isFromProject
        ? `${API_ADD_COMMON_WORK}/create-from-project`
      ...
      const recurringConfigId = response?.recurringConfigId
      ...
      if (physicalFiles.length > 0) {
        await handleUploadFiles(physicalFiles, newTaskId, 'taskdocuments');
        if (data.repeatTask !== 'khong' && recurringConfigId) {
          await handleUploadFiles(physicalFiles, recurringConfigId, 'taskdocrecurring');
        }
      }
      ...
      if (errorData?.code === "TEMPLATE_TIME_EXCEEDED") {
        ...
        setTemplateWarningInfo({
          templateName: errorData.templateName || selectedTemplateName,
    ...
    if (isFolderUpload) {
      ...
      for (const file of newFiles) {
        ...
        if (!nameValidation.valid) {
          // Nếu lỗi là do độ dài tên, ta thu thập để báo lỗi chung và không cho file này vào filteredFiles
  ...
  const handleDeleteFile = useCallback(() => {
    ...
    if (fileNode.type_file === 'link') {
      // Với link, fileNode chính là đối tượng trong uploadedFiles (theo cách convertFilesToTreeData thường làm)
      // Hoặc ta xóa dựa trên ID
  ...
  const handleConfirmReset = async () => {
    ...
    try {
      ...
      } else if (type === "jobToDocument" || type === "jobToMeeting") {
    ...
    <CustomSwipper
      title={
        <SkyFlexGap8 component="span" sx={{ display: 'inline-flex' }}>
          {(type === 'jobToMeeting' || type === 'jobToDocument') ? "Thêm mới công việc con" : title}
          ...
          <Grid container rowSpacing={3} columnSpacing={3} mb={4}>
            ...
            <Grid item xs={12} lg={8}>
              <Grid container rowSpacing={3} columnSpacing={2.5}>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <WrappedAsyncAutoComplete
                        label="Quy trình"
                        placeholder="Quy trình soạn thảo..."
                        {...field}
                ...
                {/* WEEKLY */}
                {repeatTask === 'tuan' && (
                  <WeeklyDaysGridItem item xs={12} md={6}>
                    <Controller
                      name="weekDays"
                      control={control}
                      render={({ field }) => (
                ...
                {/* ==================== RECURRING COMPONENT SPECIFIC FIELDS (LEFT COLUMN) ==================== */}
                {repeatTask && repeatTask !== 'khong' && (
                  <>
                    {/* Quý */}
                    {repeatTask === 'quy' && (
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <InputComponents
                              ...
                              placeholder="Tháng đầu"
                              {...field}
                              options={[
                                { title: 'Tháng đầu', value: 1 },
                                { title: 'Tháng giữa', value: 2 },
                                { title: 'Tháng cuối', value: 3 },
                              ]}
                              customLabel="title"
                              required
                              customValue="value"
                ...
                render={({ field }) => (
                  !(checkPermision?.directorSelectDepartment) ?
                    <WrappedAsyncAutoComplete
                      {...field}
                      label="Người chủ trì"
                      url={`${API_GET_COMMON_WORK_USER}?typeTaskUser=director&excludeId=${leaderExcludeIds}&leaderId=${getId(watchAssigner) || ""}`}
                      queryParams={["name", "email"]}
                      optionLabel="name"
                      optionValue="id"
                      optionSubLabel="parentName"
                    ...
                    <WrappedPersonOrUnitAsyncInput
                      {...field}
                      label="Người chủ trì"
                      personUrl={`${API_GET_COMMON_WORK_USER}?typeTaskUser=director&excludeId=${leaderExcludeIds}&leaderId=${getId(watchAssigner) || ""}`}
                ...
                render={({ field }) => (
                  !(checkPermision?.supporterSelectDepartment) ?
                    <WrappedAsyncAutoComplete
                      ...
                      url={`${API_GET_COMMON_WORK_USER}?typeTaskUser=supporter&excludeId=${coordinatorExcludeIds}&leaderId=${getId(watchAssigner) || ""}`}
                      queryParams={["name", "email"]}
                      optionLabel="name"
                      optionValue="id"
                      optionSubLabel="parentName"
                    ...
                    <WrappedPersonOrUnitAsyncInput
                      ...
                      label="Người phối hợp"
                      placeholder="Thêm thành viên hỗ trợ"
                      personUrl={`${API_GET_COMMON_WORK_USER}?typeTaskUser=supporter&excludeId=${coordinatorExcludeIds}&leaderId=${getId(watchAssigner) || ""}`}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <WrappedAsyncAutoComplete
                    isMulti
                    label="Người xem"
                    placeholder="Danh sách người nhận thông báo theo dõi"
                    {...field}
