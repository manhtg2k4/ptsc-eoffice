...
import { addCustomSenderUnit } from '@redux/slices/DocSendingUnitMgmt/DocSendingUnitMgmtSlice';
...
const AddSendingUnit = (props) => {
	...
	const handleSave = useCallback(async (data) => {
		try {
			...
			await dispatch(addCustomSenderUnit(payload)).unwrap();
			toast("Thêm đơn vị gửi thành công!", "success");
			if (onSuccess) {
				onSuccess();
			}
			if (setReloadData) {
			...
			const messageError = typeof error === 'string'
				? error
				: error?.message || "Lỗi khi thêm đơn vị gửi!";
	...
	return (
		<Dialog
			title={title || "THÊM MỚI ĐƠN VỊ GỬI"}
			open={open}
			onClose={handleClose}
...
export default withSharedComponents(AddSendingUnit);