...
// Removed unused SkyStyles based styled components
...
const AddMediaVideo = ({ open, onClose, onSuccess, sharedComponents }) => {
  ...
  const defaultFormValues = useMemo(
    () => ({
      ...
      description: "",
      topic: "",
      videoType: "Hiển thị lên trang chủ",
  ...
  const videoSourceType = watch("videoSourceType");
  ...
  const onSubmitForm = useCallback(
    async (data) => {
      ...
      const uploadVersion = async (file, typeSize, targetId) => {
        ...
        formData.append("object_type", "video_thumbnail");
        formData.append("object_id", targetId);
        if (typeSize) {
          formData.append("typeSize", typeSize);
        }
        await apis.post(API_UPLOAD_FILESS, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      };

      if (data.videoSourceType === "upload" && !videoFile) {
        toast("Vui lòng tải lên video", "error");
        return;
      }

      if (data.videoSourceType === "link" && !data.videoLink) {
        toast("Vui lòng nhập link video", "error");
        return;
      }
      ...
      try {
        ...
        formDataPayload.append("topic", data.topic);
        formDataPayload.append("videoType", data.videoType);

        if (imageFile) {
          formDataPayload.append("thumbnail", imageFile);
        }

        if (data.videoSourceType === "link") {
          formDataPayload.append("linkVideo", data.videoLink);
          formDataPayload.append("videoSourceType", data.videoSourceType);
        } else if (videoFile) {
          formDataPayload.append("video", videoFile);
          formDataPayload.append("videoSourceType", data.videoSourceType);
        }

        const response = await apis.post(API_MEDIA_VIDEO_CREATE, formDataPayload, {
          headers: {
        ...
        if (videoId && imageFile) {
          try {
            ...
            // Upload 3 size
            await Promise.all([
              uploadVersion(fSmall, "sizeSmall", videoId),
              uploadVersion(fMedium, "sizeMedium", videoId),
              uploadVersion(fBig, "sizeBig", videoId),
            ]);
          } catch (uploadSizesErr) {
             // Bỏ qua lỗi upload size phụ để không chặn luồng chính
          }
        }

        toast("Lưu video thành công!", "success");
  ...
  const handleVideoChange = useCallback(
    (event) => {
      const file = event.target.files?.[0];
      if (file) {
        if (file.size > 100 * 1024 * 1024) { // 100MB limit for video as example
  ...
  const renderVideoTypeField = useCallback(
    ...
    <CustomSwipper
      key={open ? "add-video-open" : "add-video-closed"}
      open={open && isReady}
      onClose={onClose}
    ...
    >
      <LoadingDialog open={isSubmitting}>
        <StyledDialogContent>
          Đang lưu video, vui lòng chờ trong giây lát...
        </StyledDialogContent>
      </LoadingDialog>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={7}>
            <MainCard>
              ...
              <SectionDescription>
                Nhập tiêu đề và các thông tin liên quan cho video này.
              </SectionDescription>
...
export default withSharedComponents(AddMediaVideo);
