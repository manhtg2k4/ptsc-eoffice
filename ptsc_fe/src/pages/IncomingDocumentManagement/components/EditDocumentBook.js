...
import { WarningText } from "@styles/IncomingDocument/IncomingDocument.style";
...
const EditDocumentBook = ({ open, bookDocumentId, onClose, onSuccess, isLoading = false, sharedComponents }) => {
  const {
		Dialog,
  ...
  // useEffect để tự động tạo Mã sổ khi Tên sổ hoặc Năm thay đổi
  useEffect(() => {
    // Chỉ tạo mã khi người dùng đã nhập tên sổ và có năm
    if (bookNameValue && yearValue) {
      const prefix = "SOVBDEN";
  ...
  if (loading) {
    return (
      <Dialog title="Chỉnh sửa Sổ văn bản" open={open} onClose={onClose} size="sm">
      ...
      <Grid container spacing={2} mt={1}>
        ...
        <Grid item xs={12}>
          ...
          />
				...
				<Grid item xs={12}>
					<WarningText>Lưu ý: Sổ sao y với sổ văn bản không được trùng nhau!</WarningText>
				</Grid>
      </Grid>
    </Dialog>
  );
...
export default withSharedComponents(EditDocumentBook);