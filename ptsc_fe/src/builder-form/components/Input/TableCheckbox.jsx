...
const TableCheckbox = ({
  ...
  return (
    <>
      <LabelTypography>{label} {required && <span style={{ color: 'red' }}>*</span>}</LabelTypography>
      {error && <StyledFormHelperText error={error}>{helperText}</StyledFormHelperText>}
      {mode === "builder" && (
        <BuilderSelect
          size="small"
        ...
        >
          {funcDataList.map((item) => (
            <StyledMenuItem key={item._id} value={item.code}>
              {item.name}
            </StyledMenuItem>
          ))}
        </BuilderSelect>
