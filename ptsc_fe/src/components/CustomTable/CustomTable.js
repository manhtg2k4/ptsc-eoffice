...
const DropdownContainer = styled(SkyPaper, {
...
// const FilterAnchorBox = styled(Box)(() => ({
// 	position: "relative",
// 	display: "flex",
// 	alignItems: "center",
// }));
...
/**
 * Tính toán khoảng ngày mặc định dựa trên cấu hình truyền vào.
 * Hỗ trợ các kiểu: "quarter" (Quý hiện tại) hoặc "months" (Số tháng gần đây)
 */
export const getDefaultDateRange = (config) => {
	if (!config || !config.type || !config.field) {
	...
	switch (config.type) {
		case "quarter": {
			...
			startDate = today.month(startMonth).startOf("month");
			endDate = today.month(endMonth).endOf("month");
			break;
		}
		case "months": {
			const value = typeof config.value === "number" ? config.value : 3;
			startDate = today.subtract(value, "month");
			endDate = today;
			break;
		}
		case "current_month": {
...
const CustomTable = ({
	...
	styledMaxHeight, // Prop để set trực tiếp calc(100vh - Xpx) cho container
	...
	lockRowsPerPageOptions = false,
	showFilterTitleIcon = true,
	hideHeaderColumnDivider = false,
	hideTableBorder = false,
	hidePaginationBorder = false,
	filterPopupAlignLeft = false,
	forceFooterFullWidth = false,
	fillHeight = false,
	footerGap = 0,
	hidePaginationWhenEmpty = false,
	defaultDateFilter = null,
	onDefaultDateRangeInit = null,
	hidePagination = false, // Ẩn toàn bộ pagination để component cha tự render
	styleTextAlignHeader,
	encodeHtml = false,
}) => {
	...
	const hasAppliedDefaultDateFilter = React.useRef(false);

	const defaultDateField = defaultDateFilter?.field;
	const defaultDateType = defaultDateFilter?.type;
	const defaultDateValue = defaultDateFilter?.value;

	const memoizedDefaultDateFilter = useMemo(() => {
		if (!defaultDateField) return null;
		return {
			field: defaultDateField,
			type: defaultDateType,
			value: defaultDateValue,
		};
	}, [defaultDateField, defaultDateType, defaultDateValue]);
	...
	const hasAddButtonLabel = useMemo(() => {
		return typeof addButtonLabel === "string" && addButtonLabel.trim().length > 0;
	}, [addButtonLabel]);
	...
	const [rowsPerPageOptions, setRowsPerPageOptions] = useState(() => {
		if (
			Array.isArray(customRowsPerPageOptions) &&
			customRowsPerPageOptions.length > 0
		) {
			return customRowsPerPageOptions;
		}
		return [25, 50, 100];
	});
	...
	const isUnitModernPreset = uiPreset === "unitModern";
	const enableModernActionColors = useModernActionColors || isUnitModernPreset;
	const shouldHideHeaderColumnDivider = hideHeaderColumnDivider || isUnitModernPreset;
	const shouldHideTableBorder = hideTableBorder || isUnitModernPreset;
	const shouldHidePaginationBorder = hidePaginationBorder || isUnitModernPreset;
		...
		if (dataViewConfig) {
			...
			if (columnsTable.length === 0 && propColumns) {
				...
				const mergedColumns = columnsTable.map((col) => {
					const matchedPropCol = (propColumns || []).find(
						(pc) =>
							(pc.row && pc.row === col.name) ||
							(pc.row && pc.row === col.row) ||
							(pc.name && pc.name === col.title) ||
							(pc.name && pc.name === col.name)
					);
					if (matchedPropCol) {
						return {
							...matchedPropCol,
							...col,
							accessor: matchedPropCol.accessor,
							row: matchedPropCol.row || col.row || col.name,
						};
					}
					return col;
				});

				// Include any columns from propColumns that are not present in columnsTable
				if (propColumns) {
					propColumns.forEach((pc, originalIndex) => {
						const isMissing = !columnsTable.some(
							(col) =>
								(pc.row && pc.row === col.name) ||
								(pc.row && pc.row === col.row) ||
								(pc.name && pc.name === col.title) ||
						...
						if (isMissing) {
							// Thử chèn vào đúng vị trí gốc
							const insertIndex = Math.min(originalIndex, mergedColumns.length);
							mergedColumns.splice(insertIndex, 0, {
								...pc,
								isShow: pc.isShow !== undefined ? pc.isShow : true,
							});
						}
					});
				}
	...
	useEffect(() => {
		if (
			...
			setRowsPerPageOptions(customRowsPerPageOptions);
			if (!customRowsPerPageOptions.includes(rowsPerPage)) {
				setRowsPerPage(customRowsPerPageOptions[0]);
				setPage(0);
			}
			return;
		}

		if (!lockRowsPerPageOptions && systemParams?.data) {
	...
	const fetchTableData = useCallback(
		async (query = "", code = [], sort, selectedType) => {
			...
			if (fetchData) {
				if (memoizedDefaultDateFilter && memoizedDefaultDateFilter.field && !hasAppliedDefaultDateFilter.current) {
					const defaultRange = getDefaultDateRange(memoizedDefaultDateFilter);
					hasAppliedDefaultDateFilter.current = true;
					if (onDefaultDateRangeInit) {
						onDefaultDateRangeInit(defaultRange);
						return; // Bỏ qua lần fetch này, để component cha cập nhật state và kích hoạt fetch đồng bộ
					}
				}
				...
				let defaultParams = {};
				if (memoizedDefaultDateFilter && memoizedDefaultDateFilter.field && !onDefaultDateRangeInit) {
					const defaultRange = getDefaultDateRange(memoizedDefaultDateFilter);
					const fieldKeyStart = `filter[${memoizedDefaultDateFilter.field}][startDate]`;
					const fieldKeyEnd = `filter[${memoizedDefaultDateFilter.field}][endDate]`;
					
					if (!advancedParams[fieldKeyStart] && !advancedParams[fieldKeyEnd] &&
						!searchParams[fieldKeyStart] && !searchParams[fieldKeyEnd]) {
						if (defaultRange.startDate) {
							defaultParams[fieldKeyStart] = formatDate(defaultRange.startDate);
						}
						if (defaultRange.endDate) {
							defaultParams[fieldKeyEnd] = formatDate(defaultRange.endDate);
						}
					}
				}
				...
				try {
					...
					logger.log("Lỗi khi gọi dữ liệu", error);
		...
		[
			...
			memoizedDefaultDateFilter,
			onDefaultDateRangeInit,
		]
	);
	...
	const shouldRenderPagination = !hidePaginationWhenEmpty || total > 0;
	...
	const renderAddButton = useCallback(
		...
		[addButtonLabel, hasAddButtonLabel, disableAdd, disableAdds, onAdd, permissionsForModule]
	);
	...
	const createRowHandlers = useCallback(
		(row) => {
			const rowId = row.documentId || row.document_id || row.id || row._id || row.code;
			return {
				...
				onSign: (e) => handleClick(e, row._id || row.documentId || row.document_id, row),
				onVerifyFormat: (e) => handleClick(e, row._id || row.documentId || row.document_id, row),
	...
	return (
		<>
			<StyleBoxTittle fillHeight={fillHeight}>
				...
				<StyledPaper isMobieHeight={isMobieHeight} isInsideDialog={isInsideDialog} autoHeight={autoHeight} fixedHeight={fixedHeight} customMaxHeight={customMaxHeight} styledMaxHeight={styledMaxHeight} disablePaperHeight={disablePaperHeight} fillHeight={fillHeight}>
					{!onlyTable && (
						<StyledToolbar>
							<ToolbarContent>
								{!disableFilter && (
									<SearchContainer>
										...
										<ClickAwayListener onClickAway={handleFilterAway}>
											{/* <FilterAnchorBox> */}
											<Box>
												<StyledFilterButtonNoBorder onClick={handleFilterToggle}>
													<TuneIcon />
												</StyledFilterButtonNoBorder>
												{openFilter && (
									...
									<AdvancedFilterContainer>
										...
										{openAdvancedFilter && (
											<ClickAwayListener onClickAway={handleCloseAdvancedFilter} mouseEvent="onMouseDown">
												<DropdownContainer alignLeft={filterPopupAlignLeft}>
													<DropdownHeader>
														Bộ lọc
														<FilterAltIcon />
													</DropdownHeader>
					...
					<StyledTableContainer isMaxHeight={isMaxHeight} customMaxHeight={customMaxHeight}>
						<StyledTable styleBorder={shouldHideTableBorder ? "none" : undefined}>
							<StyledTableHead>
								<StyledTableRow>
									{disableCheckbox ? (
										!hideSTT ? <STTHeaderCell>STT</STTHeaderCell> : null
									) : (
									...
									{visibleColumns.map((column, idx) => {
										...
										return (
											<StyledTableCell
												...
												$hideDivider={shouldHideHeaderColumnDivider}
											>
												{(() => {
													...
													const columnAlign = isSpecialColumn
														...
														: "left";
													return (
														<HeaderCellContainer align={styleTextAlignHeader ? styleTextAlignHeader : columnAlign}>
															{column.title || column.label || column.name}
								...
								{data.length === 0 ? (
									<StyledTableRow>
										<StyledTableCell
											colSpan={
												disableCheckbox
													? visibleColumns.length + (hideSTT ? 0 : 1) + (anableSTT ? 1 : 0)
													: anableSTT
									...
									data.map((row, index) => {
										...
										const rowId = row.documentId || row.document_id || row.id || row._id || row.code || index;
										const isPopoverOpen =
															...
															<Tooltip title={tooltipText || ""} placement="top" arrow>
																<StyledTableContentTolltip >
																	{isHtml && typeof displayValue === 'string' ? (
																		encodeHtml ? (
																			<div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(`<p>${encodeHTML(displayValue)}</p>`) }} />
																		) : (
															...
															// Nếu không có prop `actions`, render các nút mặc định
																...
																if (disableCheckthedigits) {
																	items.push({
																		...
																		onClick: (e) =>
																			handleClick(e, row.documentId || row.document_id || row.id || row._id, row),
																	});
																}
					...
					{!hidePagination && (!onlyTable || paginationProps) && shouldRenderPagination && (
						<ActionsContainer
							styleJustifyContent={isSmallScreen ? "flex-end" : "flex-end"}
							$isModern={isUnitModernPreset}
							$forceFullWidth={forceFooterFullWidth}
						>
							{(!isSmallScreen || paginationProps) && (
								<PaginationWrapper $footerGap={footerGap}>
									<PaginationContainerStyled
										$hideBorder={shouldHidePaginationBorder}
										$isModern={isUnitModernPreset}
									>
										{isUnitModernPreset ? (
											<>
											...
											<>
												<div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
													<span>
														Hiển thị <strong>{startRecord}-{endRecord}</strong> trong tổng số <strong>{total?.toLocaleString() || 0}</strong> {recordLabel}
													</span>
												</div>
					...
					{children}
					{!hidePagination && (!onlyTable || paginationProps) && isSmallScreen && shouldRenderPagination && (
						<ActionsContainerFooter>
							<PaginationContainer>
								{/* <span>{total}</span>
...
CustomTable.propTypes = {
	...
	styledMaxHeight: PropTypes.number,
	...
	isSettingColumn: PropTypes.bool,
	...
	uiPreset: PropTypes.oneOf(["default", "unitModern"]),
	actionIconSize: PropTypes.oneOf(["small", "medium"]),
	useModernActionColors: PropTypes.bool,
	rowsPerPageOptions: PropTypes.arrayOf(PropTypes.number),
	lockRowsPerPageOptions: PropTypes.bool,
	showFilterTitleIcon: PropTypes.bool,
	hideHeaderColumnDivider: PropTypes.bool,
	hideTableBorder: PropTypes.bool,
	hidePaginationBorder: PropTypes.bool,
	filterPopupAlignLeft: PropTypes.bool,
	forceFooterFullWidth: PropTypes.bool,
	fillHeight: PropTypes.bool,
	footerGap: PropTypes.number,
	hidePaginationWhenEmpty: PropTypes.bool,
	defaultDateFilter: PropTypes.shape({
		field: PropTypes.string.isRequired,
		type: PropTypes.oneOf(["quarter", "months"]).isRequired,
		value: PropTypes.number,
	}),
	onDefaultDateRangeInit: PropTypes.func,
};
CustomTable.propTypes = {
	...CustomTable.propTypes,
