...
import { Grid, Box, IconButton, Popover, Typography, Chip, FormHelperText } from "@mui/material";
...
import { API_ARCHIVE_FOLDER, API_GET_LIST_UNIT  } from "@EnvironmentFile/constants/urlConfig";
...
const documentBookSchema = yup.object().shape({
  bookCode: yup.string(),
  bookName: yup.string().required("Vui lòng nhập tên thư mục").max(260, "Tên thư mục không được vượt quá 260 ký tự"),
...
const Addfoder = ({
  ...
  const user = useMemo(() => {
    ...
    return dataUser.user && typeof dataUser.user === "object" ? dataUser.user : dataUser;
  }, [dataUser]);
  ...
  const helpPopoverId = useMemo(() => isHelpOpen ? 'edit-permission-help-popover' : undefined, [isHelpOpen]);
  ...
  }, [bookNameValue, yearValue, setValue, user]);
  ...
  const handleSave = handleSubmit(async (data) => {
    ...
    try {
      ...
      toast("Thêm mới thư mục thành công!", "success");
    ...
    >
      <StyledGridContainer container spacing={2}>
        ...
        <SelectIndividualsDialog
          ...
          title="CHỌN PHÒNG BAN CÓ QUYỀN CHỈNH SỬA THƯ MỤC"
...
export default withSharedComponents(Addfoder);
