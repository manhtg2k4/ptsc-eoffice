...
const SchedulerCalendar = ({ events = [], onEventClick, onDateClick, editable = false }) => {
  ...
  const handleEventClick = (clickInfo) => {
    try {
      logger.info('[SchedulerCalendar] Event Clicked:', clickInfo.event.title);
      if (onEventClick) {
        onEventClick(clickInfo.event);
      }
    } catch (error) {
      logger.error('[SchedulerCalendar] Error in handleEventClick:', error);
    }
  };

  const handleDateClick = (arg) => {
    try {
      logger.info('[SchedulerCalendar] Date Clicked:', arg.dateStr);
      if (onDateClick) {
        onDateClick(arg);
      }
    } catch (error) {
      logger.error('[SchedulerCalendar] Error in handleDateClick:', error);
    }
  };
        ...
        <FullCalendar
          ...
          customButtons={{
            customToday: {
              ...
              click: function() {
                try {
                  logger.info('[SchedulerCalendar] Today Button Clicked');
                  const calendarApi = this;
                  calendarApi.today();
                } catch (error) {
                  logger.error('[SchedulerCalendar] Error in Today button:', error);
                }
              }
            },
            customPrev: {
              ...
              click: function() {
                try {
                  logger.info('[SchedulerCalendar] Prev Button Clicked');
                  const calendarApi = this;
                  calendarApi.prev();
                } catch (error) {
                  logger.error('[SchedulerCalendar] Error in Prev button:', error);
                }
              }
            },
            customNext: {
              ...
              click: function() {
                try {
                  logger.info('[SchedulerCalendar] Next Button Clicked');
                  const calendarApi = this;
                  calendarApi.next();
                } catch (error) {
                  logger.error('[SchedulerCalendar] Error in Next button:', error);
                }
              }
            }
          }}
