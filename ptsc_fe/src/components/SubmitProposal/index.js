...
const SubmitProposal = (props) => {
	...
	const fetchData = useCallback(async () => {
		try {
			...
			const params = isAuthority
				...
				: undefined;
			await Promise.all([
				dispatch(fetchUsersDataSubmitProposal({ body: bodyUser, params })),
	...
	logger.log('subActionCodeMap', subActionCodeMap)
				...
				const specificBody =
					trueCount >= 2
						...
						: canTransferRooms === true
							...
							// 	...(newAssignments.length > 0 && { suggesteHandling: newAssignments }),
							// };
							: {
	...
	const getAssignmentRole = useCallback((assignment) => {
		if (assignment.chiDao) {
			...
			// Khối suggest: Phân biệt LĐ (Chỉ đạo) và Xử lý chính
		...
		(unitId, type, unitType, item, source = "main") => {
			...
			setAssignments((prev) => {
				...
				if (type === "chiDao") {
					...
					if (source === "suggest") {
						// khối dưới: chỉ cho 1 người chỉ đạo đề xuất
						Object.keys(updatedAssignments).forEach((k) => {
							const a = updatedAssignments[k];
                      ...
                      />
									...
									{dataMergeUserAndUnitSuggest && dataMergeUserAndUnitSuggest.length > 0 && (
										...
										<>
											...
											<StyleScrollRenderTableTree customMaxHeight="310px"  >
