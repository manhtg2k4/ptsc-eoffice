...
export const fetchTemplateCategories = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      if (query !== "" && code && sort) {
        const { data } = await api.get(`${API_GET_LIST_API}`, {
          params,
        });
        return {
          data: data?.data?.data || [],
...
export const fetchConfigurationAPIById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      // const response = await fetch(`${API_GET_LIST_API}/${id}`);
      const response = await callApi("get", `${API_GET_LIST_API}/${id}`);
...
export const fetchSearchTemplate = createAsyncThunk(
  ...
  async ({ query, code }, { rejectWithValue }) => {
    try {
      ...
      const response = await api.get(`${API_GET_LIST_API}?${queryString}`);
...
export const deleteConfigurationAPI = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await api.delete(`${API_GET_LIST_API}/${id}`);
      return id;
    } catch (error) {
...
export const updateConfigurationAPI = createAsyncThunk(
  ...
  async ({ id, updatedData }, { rejectWithValue }) => {
    try {
      const response = await api.put(`${API_GET_LIST_API}/${id}`, updatedData);
      return response.data.data;
    } catch (error) {
