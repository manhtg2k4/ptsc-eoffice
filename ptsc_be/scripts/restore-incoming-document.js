...
async function restoreIncomingDocument() {
  const backupFile = process.argv[2];
  if (!backupFile) {
    console.error('❌ Vui lòng cung cấp đường dẫn file backup. Ví dụ: node scripts/restore-incoming-document.js backup_incoming_726949903703.json');
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      const deleteQueries = [
        { table: 'incomming_current_state', column: 'document_id' },
        { table: 'incomming_assignment', column: 'document_id' },
        { table: 'incomming_suggested_handlings', column: 'document_id' },
        ...
        { table: 'incomming_documents', column: 'document_id' }
      ];

      // Lưu trữ fileIds để xóa bảng files
      const fileIds = tables['file_relations']?.map(fr => fr.file_id).filter(id => id !== null && id !== undefined) || [];
