...
async function backupIncomingDocument() {
  const documentId = process.argv[2];
  if (!documentId) {
    console.error('❌ Vui lòng cung cấp document_id. Ví dụ: node scripts/backup-incoming-document.js 726949903703');
    process.exit(1);
  }
  ...
  try {
    ...
    const tables = [
      { name: 'incomming_documents', column: 'document_id' },
      { name: 'audit', column: 'document_id' },
      { name: 'incomming_assignment', column: 'document_id' },
      { name: 'work_items', column: 'document_id' },
      { name: 'file_relations', column: 'object_id' },
      { name: 'incomming_current_state', column: 'document_id' },
      { name: 'incomming_suggested_handlings', column: 'document_id' },
    ...
    if (fileRelations && fileRelations.length > 0) {
      const fileIds = fileRelations.map(fr => fr.file_id).filter(id => id !== null && id !== undefined);
      if (fileIds.length > 0) {
        const result = await pool.request()
    ...
    const fileName = `backup_incoming_${documentId}.json`;
