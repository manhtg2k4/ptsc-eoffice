...
async function restoreMeeting() {
  ...
  if (!backupFile) {
    console.error('❌ Vui lòng cung cấp đường dẫn file backup. Ví dụ: node scripts/restore-meeting.js backup_meeting_3fa85f64-5717-4562-b3fc-2c963f66afa6.json');
    process.exit(1);
  }
  ...
  const { meeting_id, tables } = backupData;
  ...
  try {
    ...
    try {
      console.log(`\n🗑️  Đang xóa dữ liệu cũ cho meeting_id: ${meeting_id}...`);
      
      // Thứ tự xóa từ con tới cha để tránh vi phạm khoá ngoại (FK)
      
      // 1. Xóa meeting_participants (liên kết với meeting_units)
      const meetingUnits = tables['meeting_units'] || [];
      const unitIds = meetingUnits.map(u => u.id).filter(Boolean);
      if (unitIds.length > 0) {
        const formattedUnitIds = unitIds.map(id => `'${id}'`).join(',');
        
        await transaction.request().query(`DELETE FROM meeting_participants WHERE meeting_unit_id IN (${formattedUnitIds})`);
        console.log(`   - Đã xóa từ meeting_participants`);

        await transaction.request().query(`DELETE FROM meeting_unit_seats WHERE meeting_unit_id IN (${formattedUnitIds})`);
        console.log(`   - Đã xóa từ meeting_unit_seats`);
      }

      // 2. Xóa các bảng liên kết trực tiếp bằng meeting_id hoặc document_id
      const directDeleteTables = [
        { table: 'meeting_units', column: 'meeting_id' },
        { table: 'meeting_tasks', column: 'meeting_id' },
        { table: 'online_meetings', column: 'meeting_id' },
        { table: 'meeting_recurrences', column: 'meeting_id' },
        { table: 'meeting_guests', column: 'meeting_id' },
      ...
      for (const item of directDeleteTables) {
        try {
          await transaction.request()
            .input('meetingId', sql.VarChar, meeting_id)
            .query(`DELETE FROM ${item.table} WHERE ${item.column} = @meetingId`);
      ...
      // 4. Xóa bản ghi chính ở bảng meetings
      await transaction.request()
        .input('meetingId', sql.VarChar, meeting_id)
        .query(`DELETE FROM meetings WHERE id = @meetingId`);
      console.log(`   - Đã xóa từ meetings`);

      console.log(`\n📥 Đang chèn lại dữ liệu cuộc họp từ backup...`);
      ...
      for (const tableName of insertOrder) {
        ...
        // Lấy danh sách các cột computed (không thể chèn)
      ...
      console.log('\n============================================');
      console.log('✅ Khôi phục cuộc họp hoàn tất!');
      console.log('============================================');
    ...
    console.error('❌ Lỗi khi khôi phục cuộc họp:', error.message);
