...
async function restoreDestroyRecord() {
  const backupFile = process.argv[2];
  if (!backupFile) {
    console.error('❌ Vui lòng cung cấp đường dẫn file backup. Ví dụ: node scripts/restore-destroy-record.js backup_destroy_record_da4d237b-9c71-47bb-a982-fa810427339d.json');
    process.exit(1);
  }
  ...
  const { destroy_record_id, tables } = backupData;
      ...
      for (const dq of deleteQueries) {
        try {
          ...
          await transaction.request()
            .input('id', sql.VarChar, destroy_record_id)
