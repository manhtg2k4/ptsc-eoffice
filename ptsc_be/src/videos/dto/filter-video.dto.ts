...
// Danh sách các field được phép filter cho Video
export const ALLOWED_VIDEO_FILTER_KEYS: string[] = [
  'id', 'title', 'description', 'topic', 'videoType', 'video_type',
  'createdBy', 'created_by', 'createdByName', 'created_by_name',
  'createdAt', 'created_at', 'updatedAt', 'updated_at',
  'publishedDate', 'published_date', 'publishedAt', 'published_at',
  'views', 'shares', 'likes', 'status', 'keyword', 'isStar',
...
// --- Validator filter object cho Video
export function IsValidVideoFilter(validationOptions?: ValidationOptions) {
  return function (object: object, propertyName: string) {
    registerDecorator({
      name: 'IsValidVideoFilter',
        ...
        validate(filter: any) {
          ...
          function validateRecursively(obj: any, path = '') {
            for (const key of Object.keys(obj)) {
              ...
              const allowed = ALLOWED_VIDEO_FILTER_KEYS.some(
...
// --- Validator sort object cho Video
export function IsValidVideoSort(validationOptions?: ValidationOptions) {
  return function (object: object, propertyName: string) {
    registerDecorator({
      name: 'IsValidVideoSort',
      target: object.constructor,
      propertyName,
      options: validationOptions,
      validator: {
...
/**
 * DTO cho API list videos
 * Hỗ trợ nested filter object, paging, sort theo format của documents
 */
export class FilterVideoDto {
  @ApiPropertyOptional({
    description: 'Filter object - hỗ trợ các field: title, topic, videoType, createdBy, createdByName, keyword. Hỗ trợ date range với format { createdAt: { startDate: "YYYY-MM-DD", endDate: "YYYY-MM-DD" } }',
    example: { keyword: 'test', topic: 'uuid', videoType: 'featured', createdAt: { startDate: '2025-01-01', endDate: '2025-12-31' } }
  })
  @IsOptional()
  @IsObject()
  @IsValidVideoFilter()
