...
export class FileSignDto {
    ...
    @ApiProperty({ description: 'Base64 file PDF' })
    @IsNotEmpty()
    base64: string;
    ...
    @ApiProperty({ description: 'Loại file', example: 'application/pdf' })
    @IsString()
    @IsOptional()
    mimeType?: string;
}
export class SignFileOtpDto {
    ...
    @ApiProperty({ description: 'Signed file buffer (base64) - FE đã ký USB, gửi buffer lên BE để upload', required: false })
    @IsString()
    @IsOptional()
    signedFileBuffer?: string;
    ...
    @ApiProperty({ description: 'Ảnh dấu (base64)', required: false })
    @IsString()
    @IsOptional()
